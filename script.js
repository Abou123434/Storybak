...
function showStory(){
    ...
    /* boutons */
    let old=document.getElementById("progressControls"); if(old) old.remove();
    let controls=document.createElement("div");
    controls.id="progressControls";
    controls.style.position="absolute"; controls.style.top="35px";
    controls.style.left="10px"; controls.style.right="10px";
    controls.style.display="flex"; controls.style.justifyContent="space-between";
...
function openGiftQuantityModal(){
    let modal=document.createElement("div"); modal.style.position="fixed"; modal.style.inset="0";
    modal.style.background="rgba(0,0,0,0.9)"; modal.style.display="flex";
    modal.style.justifyContent="center"; modal.style.alignItems="center"; modal.style.zIndex="9999";
    let box=document.createElement("div"); box.style.background="#111"; box.style.padding="25px";
    box.style.borderRadius="15px"; box.style.textAlign="center";
    box.innerHTML=`
...
function sendGift(q){
    ...
    if((coins[currentProfile.username]||0)>=total){
        ...
        document.getElementById("giftMessage").innerText=`Cadeau envoyé ${selectedGiftEmoji
