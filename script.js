...
function showStory() {
    ...
    if (currentProfile.username === currentUser) {
        let delBtn = document.createElement("button"); delBtn.innerText = "Supprimer"; delBtn.onclick = () => {
            if (confirm("Supprimer cette story ?")) { users[currentUser].stories.splice(currentIndex, 1); saveData(); if (users[currentUser].stories.length === 0) { closeViewer(); return; } showStory(); }
...
const walletBuyBtn = document.getElementById("walletBuyCoins");
...
walletBtn.onclick = () => {
    ...
    walletDiamonds.innerText = 8400; // Exemple
...
document.getElementById("hamburger").onclick = () => {
    let m = document.getElementById("menuOptions"); m.style.display = m.style.display === "flex" ? "none" : "flex";
};
