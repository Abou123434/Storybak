...
function showStory(){
    ...
    if(currentProfile.username===currentUser){
        ...
        delBtn.onclick=()=>{ if(confirm("Supprimer cette story ?")){ users[currentUser].stories.splice(currentIndex,1); saveData();
...
const kycSendBtn=document.getElementById("kycSendBtn");
...
withdrawBtn.onclick=()=>{
    ...
    kycPaypalBtn.style.display="none";
    kycMessage.innerText="Veuillez envoyer vos documents pour vérifier votre identité.";
};

/* KYC SIMULATION */
kycSendBtn.onclick=()=>{
    if(!kycFile.files[0]){ alert("Veuillez sélectionner un document."); return; }
    kycStatus.style.display="block"; kycMessage.innerText="Vérification en cours ⏳";
    kycSendBtn.style.display="none";

    setTimeout(()=>{
        kycMessage.innerText="KYC validé ✅";
        kycPaypalBtn.style.display="block";
