...
function generateAvatar(nom,prenom){
    ...
    let ctx=c.getContext("2d"); ctx.fillStyle="#25D366"; ctx.fillRect(0,0,150,150);
...
function addImage(file){ let r=new FileReader(); r.onload=e=>{ users[currentProfile.username].stories.push({url:e.target.result,type:"image",views:{}}); saveData(); renderStories(); }; r.readAsDataURL(file); }
...
function openViewer(u){
    ...
    hideMainMenu(); showBosteAndPublish();
    showStory();
}
...
function showStory(){
    ...
    let e=s.type==="image"?document.createElement("img"):document.createElement("video"); e.src=s.url; if(s.type==="video") e.autoplay=true; c.appendChild(e);
...
function startProgress(s){
    let bars=document.querySelectorAll(".progress-inner"); let w=0; let dur=s.type==="image"?5000:10000;
    timer=setInterval(()=>{ w+=100/(dur/50); bars[currentIndex].style.width=Math.min(w,100)+"%";
    if(w>=100){ clearInterval(timer); if(currentIndex<users[currentUser].stories.length-1){ currentIndex++; showStory(); } else closeViewer(); } },50);
}
...
function closeViewer(){ clearInterval(timer); document.getElementById("viewer").style.display="none"; hideBosteAndPublish(); showMainMenu(); }
...
// Boste & flèche
const bosteBtn=document.getElementById("bosteBtn");
const publishBtn=document.getElementById("publishBtn");
function showBosteAndPublish(){
    bosteBtn.style.display="block"; publishBtn.style.display="block";
    bosteBtn.onclick=()=>{}; // inclicable
    publishBtn.onclick=()=>{
        alert("Story publiée !");
        hideBosteAndPublish();
    };
}
function hideBosteAndPublish(){ bosteBtn.style.display="none"; publishBtn.style.display="none"; }

// Hamburger
function hideMainMenu(){ document.getElementById("hamburgerContainer").style.display="none"; }
function showMainMenu(){ document.getElementById("hamburgerContainer").style.display="block"; }
