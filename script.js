...
function generateAvatar(nom,prenom){
    ...
    ctx.fillText(nom[0]+prenom[0],75,75); return c.toDataURL();
}
...
function addVideo(f){let url=URL.createObjectURL(f); users[currentProfile.username].stories.push({url, type:"video", views:{}}); saveData(); renderStories();}
...
function openGiftQuantityModal(){
    ...
    let box=document.createElement("div"); box.style.cssText="background:#111;padding:25px;border-radius:15px;text-align:center;"; box.innerHTML=`
