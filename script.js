...
function openViewer(u){
    ...
    currentUser=u; currentIndex=0; document.getElementById("viewer").style.display="flex"; showStory();
}
...
function showStory(){
    ...
    if(currentProfile.username===currentUser){
        ...
        delBtn.onclick=()=>{ if(confirm("Supprimer cette story ?")){ users[currentUser].stories.splice(currentIndex,1); saveData(); if(users[currentUser].stories.length===0){ closeViewer(); return;} showStory(); }};
...
function sendGift(q){ let t=selectedGiftCost*q;
...
withdrawBtn.onclick=()=>{ kycModal.style.display="flex"; };
...
submitKYC.onclick=()=>{
    ...
    kycMessage.innerText="✅ Vérification envoyée !\nTemps estimé : 5 minutes à 24 heures";
    withdrawPaypalBtn.style.display="inline-block";
    setTimeout(()=>kycModal.style.display="none",2000);
};
