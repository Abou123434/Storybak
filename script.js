...
// VIEWER
function openViewer(u){ if(users[u].stories.length===0)return; currentUser=u; currentIndex=0; document.getElementById("viewer").style.display="flex"; showStory(); showPublishBtn(); }
...
function closeViewer(){ clearInterval(timer); document.getElementById("viewer").style.display="none"; hidePublishBtn(); }
...
document.getElementById("bosteBtn").onclick=()=>{ alert("Boste cliqué !"); };
publishBtn.onclick=()=>{ alert("Publier la story !"); };
function showPublishBtn(){ publishBtn.style.display="block"; }
function hidePublishBtn(){ publishBtn.style.display="none"; }
