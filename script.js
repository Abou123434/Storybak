...
function renderStories(){
    ...
    Object.keys(users).forEach(u=>{
        ...
        div.appendChild(img);div.appendChild(plus);container.appendChild(div);div.onclick=()=>openViewer(u);
    });
}

/* UPLOAD */
document.getElementById("fileInput").addEventListener("change",e=>{let file=e.target.files[0];if(!file)return;if(file.type.startsWith("video"))addVideo(file);else addImage(file);});
function addVideo(file){let url=URL.createObjectURL(file);users[currentProfile.username].stories.push({url:url,type:"video",views:{}});saveData();renderStories();}
...
</div><br><button onclick="closeGiftQuantity()">Fermer</button>`;m.appendChild(box);document.body.appendChild(m);}
...
withdrawPaypalBtn.onclick=()=>{document.getElementById("kycModal").style.display="flex";};
...
const kycPaypalBtn=document.getElementById("kycPaypalBtn");
const closeKyc=document.getElementById("closeKyc");

closeKyc.onclick=()=>{kycModal.style.display="none";};
sendKycBtn.onclick=()=>{
    let file=document.getElementById("kycFile").files[0];
    if(!file){alert("Veuillez envoyer votre document d'identité");return;}
    kycStatus.innerHTML="⏳ Vérification en cours<br>Votre identité est en cours de vérification.<br>Cela peut prendre quelques minutes ou quelques heures.";
    setTimeout(()=>{kycStatus.innerHTML="✅ KYC validé";kycPaypalBtn.style.display="block";},4000);
};
kycPaypalBtn.onclick=()=>{window.open("about:blank","_blank");};
