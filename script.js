...
let coins = JSON.parse(localStorage.getItem("userCoins")) || {}; // stock solde
...
function register(){
  ...
  if(!nom || !prenom){ alert("Entrez nom et prénom"); return; }
  if(!pin || !/^\d{4}$/.test(pin)){ alert("Entrez un PIN valide (4 chiffres)"); return; }

  let username = nom+"_"+prenom;
  if(users[username]){ alert("Compte existe déjà"); return; }

  if(file){
    let reader=new FileReader();
    reader.onload=function(e){ createUser(username,e.target.result,pin); };
    reader.readAsDataURL(file);
  } else {
    createUser(username, generateAvatar(nom, prenom), pin);
  }
}

function createUser(username, photo, pin){
  users[username] = { photo:photo, pin: pin, stories:[], reactions:{} };
  if(!coins[username]) coins[username] = 100; // solde initial 100 coins
  saveData(); saveCoins();
...
/* CONNEXION */
function login(){
  let nom = document.getElementById("loginNom").value.trim();
  let prenom = document.getElementById("loginPrenom").value.trim();
  let pin = document.getElementById("loginPin").value.trim();

  if(!nom || !prenom || !pin){ alert("Entrez toutes les informations"); return; }

  let username = nom+"_"+prenom;
  if(!users[username]){ alert("Utilisateur non trouvé"); return; }
  if(users[username].pin !== pin){ alert("PIN incorrect"); return; }

  if(!coins[username]) coins[username] = 100; // solde initial
  saveCoins();
...
function forgotPin(){
  ...
  alert("Votre PIN est : " + users[username].pin);
}
