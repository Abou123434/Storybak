...
function renderStories(){
    ...
    allUsers.forEach(u=>{
        ...
        avatarDiv.style.border="3px solid #25D366";
        ...
        div.onclick = ()=>{
            if(u===currentProfile.username){
                currentUser = u;
                currentIndex = users[u].stories.length;
...
document.getElementById("fileInput").addEventListener("change", e=>{
    ...
    if(type==="video"){
        ...
        reader.onload = ev=>{ pendingStory = {url:ev.target.result,type,views:{}}; previewPendingStory(); };
...
function previewPendingStory(){
    ...
    let confirmBtn=document.createElement("button"); confirmBtn.innerText="Publier";
    confirmBtn.onclick=()=>{ users[currentUser].stories.push(pendingStory); saveData(); renderStories(); pendingStory=null; closeViewer(); };
    controls.appendChild(confirmBtn);
    let cancelBtn=document.createElement("button"); cancelBtn.innerText="Annuler";
    cancelBtn.onclick=()=>{ pendingStory=null; closeViewer(); };
...
function showStory(previewMode=false){
    ...
    if(previewMode || currentIndex>=users[currentUser].stories.length){
        ...
        div.innerText="Choisissez une image ou vidéo pour publier votre story"; c.appendChild(div);
    } else {
    ...
    renderProgressBars(); startProgress(users[currentUser].stories[currentIndex] || {type:"image"});
    let controls=document.getElementById("progressControls"); controls.innerHTML="";
